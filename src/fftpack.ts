...
function cfftf1(n: number, c: number[], ch: number[], wa: number[], ifac: number[], isign: number): number[] {
    ...
    for (let k1 = 2; k1 <= nf + 1; k1++) {
        ...
        //console.log('====>', k1 - 2, ip, ido)
        switch (ip) {
            case 4:
                ix2 = iw + idot
                ix3 = ix2 + idot
                passf4(idot, l1, cinput, coutput,
                    wa.slice(iw, iw + idot),
...
function passf2(ido: number, l1: number, cc: number[], ch: number[], wa1: number[], isign: number) {
    //console.log('cc(2)', cc)
    //console.log('ch(2)', ch)
    if (ido <= 2) {
        for (let k = 0; k < l1; k++) {
...
function passf3(ido: number, l1: number, cc: number[], ch: number[], wa1: number[], wa2: number[], isign: number) {
    const taur = -0.5;
    const taui = Math.sqrt(3.0) / 2
    if (ido == 2) {
        for (let k = 1; k <= l1; k++) {
            let ac = (3 * k - 2) * ido;
            let tr2 = cc[ac] + cc[ac + ido];
...
function passf4(ido: number, l1: number, cc: number[], ch: number[],
    wa1: number[], wa2: number[], wa3: number[], isign: number) {
    //console.log('l1(4)', l1)
    //console.log('cc(4)', cc, l1)
    //console.log('ch(4)', ch)
    if (ido == 2) {
        for (let k = 0; k < l1; k++) {
            let ac = 4 * k * ido + 1
            ...
            let ah = k * ido
            ch[ah] = tr2 + tr3;                            // 0   0.re
            ch[ah + 2 * l1 * ido] = tr2 - tr3;             // 4   2.re
            ch[ah + 1] = ti2 + ti3;                        // 1   0.im
            ch[ah + 2 * l1 * ido + 1] = ti2 - ti3;         // 5   2.im
            ch[ah + l1 * ido] = tr1 + isign * tr4;         // 2   1.re
            ch[ah + 3 * l1 * ido] = tr1 - isign * tr4;     // 6   3.re
            ch[ah + l1 * ido + 1] = ti1 + isign * ti4;     // 3   1.im
            ch[ah + 3 * l1 * ido + 1] = ti1 - isign * ti4; // 7   4.im
        }
    } else {
        for (let k = 0; k < l1; k++) {
            for (let i = 0; i < ido - 1; i += 2) {
...
function passf5(ido: number, l1: number, cc: number[], ch: number[],
...
function passf(ido: number, ip: number, l1: number, idl1: number, cc: number[], ch: number[], wa: number[], isign: number) {
    let nac
    let idot = ido / 2
    let ipph = (ip + 1) / 2
    let idp = ip * ido;
...
function cffti(n: number): { wa: number[], ifac: number[] } {
    if (n == 1)
        return
    return cffti1(n)
}
...
function cffti1(n: number): any {
    let wa = []
    let ifac = factorize(n) // [ input, number_of_factors, factors...]
    let nf = ifac[1] // Number of Factors

    const argh = 2.0 * Math.PI / n
